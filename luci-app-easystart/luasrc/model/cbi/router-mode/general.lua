...
current_mode.value = function()
    local status = luci.http.getenv('REQUEST_URI'):match('/admin/network/router%-mode/status')
    if status then
        local data = luci.sys.exec('curl -s http://localhost/cgi-bin/luci/admin/network/router-mode/status')
...
function apply.write(self, section, value)
    ...
    -- 调用后端脚本
    local url = '/cgi-bin/luci/admin/network/router-mode/apply'
...
function restore.write(self, section, value)
    local url = '/cgi-bin/luci/admin/network/router-mode/apply?mode=restore'
